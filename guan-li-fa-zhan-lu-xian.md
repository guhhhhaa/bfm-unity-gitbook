...
BFM Unity 将让市场上一部分成规模的交易者成为BFM Unit\(s\)。
...
BFM Unity 将让市场上1/3以上交易者成为BFM Unit\(s\)。

## 4,「伪典·天击」：复制了战神赐予天翼种的力量——天击制成的兵装。

「伪典·天击」阶段，BFM Unity 将真正向全市场宣告 BFM Unity 的影响力。  
BFM Unity 将组织 BFM Unit\(s\) 通过在同一时间停止套利系统运行，引发市场闪崩\(flash crash\)。  
此前 BFM Unity 将通知 BFM Unit\(s\) 提前持有看跌期权以获得盈利。
